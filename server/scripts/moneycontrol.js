...
async function getSym(symbols) {
    for(const sym of symbols) {
        console.log(sym);
        await rp({
            uri: `https://www.moneycontrol.com/mccode/common/autosuggestion_solr.php?classic=true&query=${sym}&type=1&format=json`,
            json: true,
        })
		    .then(function(res) {
		        data += `${sym}, ${res[0].sc_id}\n`;
		    })
		    .catch(function(err) {
		        console.log(err);
		    });
    ...
    console.log('wirting to file');

    fs.writeFile('moneycontrol_symbols.txt', data, function(err) {
	  if(err) throw err;
